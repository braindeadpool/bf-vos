...
def validation_loss(anchor_points, positive_pool, negative_pool):
    ...
    val_loss = torch.sum(torch.gt(p_, n_)).float() / float(N * M * L)
    # repeat copies the data, so we free it to not use up memory after it goes out of scope (potential bug in pytorch)
    return val_loss
